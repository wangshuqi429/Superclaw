...
class SafeFixer:
    ...
    def create_fix_commits(self, client: GitHubClient, repository: str, pull_request: int, report: dict) -> dict:
        ...
        branch = "superclaw/fix-pr-%d-%s" % (pull_request, stamp)
        planned = []
        by_path = {}
        ...
        commit = client.create_atomic_commit(
            ...
            "fix: apply verified Superclaw repairs for PR #%d" % pull_request,
        )
        draft = client.create_draft_pull_request(
            repository,
            "fix: verified Superclaw repairs for #%d" % pull_request,
